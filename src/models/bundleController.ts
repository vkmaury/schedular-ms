...
export const createBundle = async (req: Request, res: Response) => {
  try {
    ...
    // Check if all products are active and belong to the seller
  ...
  try {
    ...
    await bundle.save();
     ...
     for (const cart of carts) {
       cart.items = cart.items.filter(item => item.bundleId?.toString() !== id);
       await cart.save();
     }
     ...
     for (const wishlist of wishlists) {
       wishlist.items = wishlist.items.filter(item => item.bundleId?.toString() !== id);
       await wishlist.save();
     }
